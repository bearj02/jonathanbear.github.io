...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int size_impl(const Node *node) {
    ...
    // count 1 each time you go down and add them all together 
    //check? 
  ...
  static int height_impl(const Node *node) {
    ...
    if(node == nullptr){
      ...
      height_1 = height_impl(node->right);  // add plus 1? 
      height_2 = height_impl(node->left);   
    ...
    //compare and go to longest path in the tree
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    //part of big three
    //allocated new nodes that have the same datum and tree
    //use new keyword
    //return * to the root of the new tree
    //start small, single node, no nodes 
    //node and one left child, how would I connect them together, they have to be connected
    //parent and child node, connect them together 
  }
  ...
  static void destroy_nodes_impl(Node *node) {
    ...
    //part of big three
    //destructor 
    //delete every single node, clear out the tree
    //do not delete the parent node first, start from each side and then delete
  }
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    ...
    //tree is sorted
    //look for when the root is smaller-> so it would go to the right only, recurse right 
    //when it is equal, return node; 
    //look for when root is bigger -> look left, recurse left
    //hit nullptr, it is not found, return nullptr; 
  }
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    ...
    //can not support duplicates 
    //just like find, find which place exactly it would fit. 
    //going left or right, less than root or greater than root
    //base case is nullptr, where you want to insert 
    //create a node with new, link it to the tree properly 
    //make sure that it has the link from parent, saying it is child node 
  }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if(node->left == nullptr && node->right == nullptr){
      ...
      return !less(node->datum, max_element_impl(node->left)->datum);
    }
    else if(less(node->datum, node->left->datum) || !less(node->datum, node->right->datum)){
      return false;
    }
    if(less(node->datum, max_element_impl(node->left)->datum) || less(min_element_impl(node->right)->datum, node->datum)){
      return false;
    }
    //return between two only if both are true it will be true and if one false it will be false
    return check_sorting_invariant_impl(node->left,less) && check_sorting_invariant_impl(node->right,less); 


    //check all the way down each side 
    //left < root and right > root 
    //base case is if it hits nullptr without any errors at the end then return true; 
    //is there a function for BST that returns some property that would
    //ensure an entire tree is less than or greater than a certain value
    //something we implemented, maybe find? idk
  }
  ...
  static void traverse_inorder_impl(const Node *node, std::ostream &os) {
    ...
    //goes down the the left  one first and then prints what was before that in right ?
    //tprinitng, traverse left or traverse right click in 
    // start left I think
  }
  ...
  static void traverse_preorder_impl(const Node *node, std::ostream &os) {
    ...
    //same as before which side will you go on 
  }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    if(node == nullptr){
      ...
      Node * left_check = min_greater_than_impl(node->left,val,less); //not sure 
    ...
    else // equal has to be one to the left
    {
    ...
    //min greater than 6 is 7 
    //look for which side to do it , if root is less then recurse right 
    //if it is equal, then look to the rught
    //did not find anythinh that was greater than val, then  return  nullptr
    //if it is greater (root meaning it), then is it the minimum, look to the left if the left is hreater than 6 then keep looking
    //until that is false or nullptr
    //is left check returns nullptr , then nothing  better
    //if valid node then found sonething better
    //return left check
    //return the current root or node
    //if (nullptr){
    // we did not find anything greater than val
    //else if(node->datum < val) return to the right
    //else if node -> == val _. return to the right
    //else we have a val that is > then val, potential solution 
    // node * left check = recurse into the left subtree
    //if left check is a nullptr then didnt find anything better and return nodeptr
    //else return the left check
    //39 minutes
  }
