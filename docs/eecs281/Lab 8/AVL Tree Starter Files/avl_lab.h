...
AVL::Node* AVL::rotate_left(AVL::Node* node) {
    ...
    //if(temp->balance() > 0) temp = rotate_right(temp);
    /*if(temp->left_height() > 0 && temp->right_height() > 0){
        node->right = temp->left;
        node->right->fix_height();
        node->fix_height();
        temp->left = nullptr;
...
AVL::Node* AVL::rotate_right(AVL::Node* node) {
    ...
    //if(temp->balance() < 0) temp = rotate_left(temp);
    /*if(temp->right_height() > 0 && temp->left_height() > 0){
        node->left = temp->right;
        node->left->fix_height();
        node->fix_height();
        temp->right = nullptr;
