...
void Image_print(const Image* img, std::ostream& os) {
  ...
  for(int row = 0; row < Image_height(img); row++){
      ...
      Pixel element = Image_get_pixel(img,row,column);
      os << element.r << " " << element.g << " " << element.b << " "; 
    }
    os << "\n";
  } 
}
...
Pixel Image_get_pixel(const Image* img, int row, int column) {
  ...
  int red_data = *Matrix_at(&(img->red_channel), row, column);
  int green_data = *Matrix_at(&(img->green_channel), row, column);
  int blue_data = *Matrix_at(&(img->blue_channel), row, column);
  Pixel element = {red_data, green_data, blue_data};
